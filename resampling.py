# -*- coding: utf-8 -*-
"""
Created on Thu May 18 18:50:26 2023
...
def adap(x,t):
    ...
    while True:    # number of refinement
        ...
        if dist<1:  #again this should be the minimum distance
            a=np.array([fppnew[0]*0.05,fppnew[1]*300])
...
def resam(fp):
    ...
    for i in range(len(fp)):
        ...
        gen=np.array(adap(fp[i,0],fp[i,1]))
    ...
    #to unbox gen from list to numpy, dont know why i used ada as zero numpy instead of making it just a numpy to list
    ada=np.zeros([0,2])  #okay simply converting list to numpy wont work
...
init = tf.compat.v1.global_variables_initializer()
sess.run(init)   
target=200  #points to be selected for error refinement
adn=200
k=2
c=0
losstot=[]
...
for it in range(1,nIter):
    ...
    if it%1==0:
       ...
       #actual points which got resampled
        ...
        tt_new=np.concatenate((tt_f,sortad[:,1:2]),0)  #it doesnt take the previous refined points
        ...
        sortadind=np.argsort(abs(r_vala[:,0]))
        #here we are selecting some points whose index corresponds to higher error
        sortad=np.zeros((100,2))
        sortad=adappts[sortadind[-100:],:]
...
unq, cnt = np.unique(resamp, axis=0, return_counts=True)
 
cm=np.concatenate((unq[:,0:1],unq[:,1:2],cnt[:,None][:,0:1]),axis=1)
 


plt.scatter(xx_f,tt_f, marker='.',s=150,facecolors='None',edgecolors='tab:blue')
plt.scatter(cm[:,0],cm[:,1], c=cm[:,2],marker='.',s=150,cmap='viridis',label=c)
plt.colorbar(ticks=[0,2,4,6,8,10,12])
...
plt.scatter(xx_f,tt_f, marker='.',s=150,facecolors='tab:blue')
...
#plt.scatter(cm[:,0],cm[:,1], c=cm[:,2],s=150)
...
target=1 #points to be selected for error refinement
adn=200

refine=1
resamp=np.array([[0.3,3900]])

refinep=resam(resamp)   #here gen_fin is list of refined point for each oriningal point, and aa is the list of all those refined points

#for plotting the zoom image
plt.scatter(xx_f,tt_f, marker='.',s=1200)
#plt.scatter(xx_new,tt_new, marker='.')
plt.scatter(resamp[:,0],resamp[:,1],facecolors='none', edgecolors='r', s=1200)
plt.scatter(refinep[:,0],refinep[:,1], marker='.', facecolors='green', s=1200)
#plt.scatter(adappts[:,0],adappts[:,1], marker='.', facecolors='black')
#plt.scatter(sortad[:,0],sortad[:,1], marker='.', facecolors='black', s=500)
#plt.ylim(0, 5000)
plt.ylim(3500,4300)
plt.xlim(0.22, 0.38)
plt.show()
...
result=np.concatenate((xx_f[:,0:1],tt_f[:,0:1],orig[:,0:1]),axis=1)

import pandas as pd
df = pd.DataFrame(result)

df.to_csv("data_15000orig.csv", index=False, header= False)

import pandas as pd
df = pd.DataFrame(np.log10(np.array(losstot)))

df.to_csv("./loss/resamp.csv", index=False, header= False)

#error analysis


xv=np.linspace(0, 1, num=101)

tv=np.linspace(0, 6000, num=301)

xxv,ttv=np.meshgrid(xv,tv)
xx_v = xxv.flatten()[:,None] # NT x 1
tt_v = ttv.flatten()[:,None] # NT x 1


tf_dictv= {x_dcb: x_lb, t_dcb: t_lb, x_neb: x_rb, t_neb: t_rb, x_i: x_ic, t_i: t_ic, x_f: xx_v, t_f: tt_v}

origv=c_f.eval(feed_dict=tf_dictv,session=sess)

resultv=np.concatenate((xx_v[:,0:1],tt_v[:,0:1],origv[:,0:1]),axis=1)


comr = pd.read_csv("datacomsolpost.csv")

comrnp=comr.iloc[:,:].values

l2err=(comrnp[:,2]-resultv[:,2])**2

import math
rel_l2err=math.sqrt(np.sum(l2err))/np.sum(comrnp[:,2])
print(rel_l2err)
"""
...
fig = plt.imshow(Z,extent=[np.min(x)*6000,np.max(x)*6000,np.min(y),np.max(y)])
