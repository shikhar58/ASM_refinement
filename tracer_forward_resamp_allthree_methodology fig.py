...
init = tf.compat.v1.global_variables_initializer()
sess.run(init)   
target=50  #points to be selected for error refinement
k=2
c=0
losstot=[]
nIter=1000
...
for it in range(1,nIter):
    ...
    if it%200==0:
       ...
       #actual points which got resampled
        ...
        sortad=np.zeros((50,2))
        sortad=adappts[sortadind[-50:],:]
...
plt.scatter(xx_f,tt_f-1000, marker='.',s=200)
#plt.scatter(xx_new,tt_new, marker='.')
plt.scatter(resamp[:,0],resamp[:,1]-1000,facecolors='none', edgecolors='r',s=200)
plt.scatter(refinep[:,0],refinep[:,1]-1000, marker='.', facecolors='green',s=200)
#plt.scatter(adappts[:,0],adappts[:,1], marker='.', facecolors='black')
plt.scatter(sortad[:,0],sortad[:,1]-1000, marker='.', facecolors='black',s=200)
plt.ylim(0, 5000)
